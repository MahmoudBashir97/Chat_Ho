...
var options = {
    method: 'POST',
    uri: 'http://api.posttestserver.com/data',
    body: {
        some: 'payload'
    },
...
var options = {
    method: 'POST',
    uri: 'http://posttestserver.com/data.php',
    form: {
...
- Plus some methods on a request call object:
	- `rp(...).then(...)` or e.g. `rp.data(...).then(...)` which turn `rp(...)` and `rp.data(...)` into promises
...
The objects returned by request calls like `rp(...)` or e.g. `rp.data(...)` are regular Promises/A+ compliant promises and can be assimilated by any compatible promise library.
...
request.data('http://example.com/api', function (err, response, body) { ... });
rp.data('http://example.com/api').then(...);
```
